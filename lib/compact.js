...
const compact = async f => {
  ...
  let objs = []
  let reader = f.pipe(jsonstream.parse()).pipe(new PassThrough({objectMode: true}))
...
const compactAll = async datadir => {
  for (let year of await fs.readdir(datadir)) {
    if (isNaN(parseInt(year))) continue
    for (let month of await fs.readdir(join(datadir, year))) {
      const compacts = []
      for (let day of await fs.readdir(join(datadir, year, month))) {
